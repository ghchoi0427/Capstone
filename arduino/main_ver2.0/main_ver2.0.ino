...
//________________________________________________________________________________ 스프링클러, 조명 컨트롤
#define HEATPIN 6
#define WATERPIN 7
...
void loop() {
  ...
  String targetTemp = "-1";
  String serialRead = "";
  int currentTemp = (int((iOBJECT) / 100))
  ...
  if (Serial.available() > 0) {
    ...
    if (indexSplit != -1) {
      ...
      targetTemp = serialRead.substring(indexSplit + 1, serialRead.length() - 1);
    }

    if (currentTemp < targetTemp.toInt() - threshold) {
      digitalWrite(HEATPIN, HIGH);
      digitalWrite(WATERPIN, LOW);
    }

    else if (currentTemp > targetTemp.toInt() + threshold) {
      digitalWrite(HEATPIN, LOW);
      digitalWrite(WATERPIN, HIGH);
    }

    else if (targetTemp.toInt() - threshold <= temp.toInt() &&
             temp.toInt() <= targetTemp.toInt() + threshold) {
      digitalWrite(HEATPIN, LOW);
      digitalWrite(WATERPIN, LOW);
    }
  }
